...
Batman.ReactMixin = {
  ...
  enumerate: function(setName, itemName, generator) {
    ...
    var components = set.map(function(item){
      ...
      iterationComponent = Batman.IterationComponent(innerProps)
      iterationComponent._observeContext(innerContext)
      return generator.call(iterationComponent)
    })
  }
}
...
Batman.IterationComponent = Batman.createComponent({
  render: function() {}
})
...
App.AnimalsIndexComponent = React.createClass({displayName: 'AnimalsIndexComponent',
  ...
  allAnimals: function() {
    var component = this
    var collection =  this.sourceKeypath("animals") || []
    var items = collection.map(function(animal) {
      return(
        React.DOM.li(null ,
          React.DOM.div( {className:"row"},
            React.DOM.div( {className:"col-xs-3"},
              React.DOM.p( {className:"lead"}, animal.get('name'))
            ),
            React.DOM.div( {className:"col-xs-2"},
              React.DOM.button( {onClick:component.executeAction("edit", animal), className:"btn"}, "Edit in Dialog")
            ),
            React.DOM.div( {className:"col-xs-2"},
              React.DOM.button( {onClick:component.executeAction("destroy", animal), className:"btn btn-danger"}, "Destroy")
            )
          )
        )
        )
    ...
    return (
      React.DOM.div( {className:"row"},
        ...
        React.DOM.ul( {className:"list-unstyled"},
          /* animals */
          this.enumerate("animals", "animal", function() {
            return (
              React.DOM.li(null,
                React.DOM.div( {className:"row"},
                  React.DOM.div( {className:"col-xs-3"},
