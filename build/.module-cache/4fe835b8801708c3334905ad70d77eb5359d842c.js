...
App.AnimalsIndexComponent = React.createClass({displayName: 'AnimalsIndexComponent',
  ...
  allAnimals: function() {
    var component = this
    var items = this.sourceKeypath("animals").forEach(function(animal) {
      return(
        React.DOM.li(null , 
          React.DOM.div( {className:"row"}, 
            ...
            React.DOM.div( {className:"col-xs-2"}, 
              React.DOM.button( {onClick:component.executeAction("destroyAnimal", animal), className:"btn btn-danger"}, "Destroy")
            )
          )
        )
        )
