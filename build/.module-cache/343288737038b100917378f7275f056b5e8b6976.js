    ...
    var animals = this.enumerate("animals", "animal", function(animal) {
              ...
              React.DOM.li( {key:animal.get('_batmanID')}, 
                React.DOM.div( {className:"row"}, 
                  ...
                  React.DOM.div( {className:"col-xs-2"}, 
                    React.DOM.a( {href:this.linkTo("routes.animals[animal]"), className:"btn btn-link"}, "View")
                  ),
                  React.DOM.div( {className:"col-xs-2"}, 
              ...
              this.enumerate('Animal.CLASSES', 'animalClass', function(animalClass){
                return (
                  React.DOM.label( {className:"radio-inline"}, 
                    animalClass,
                    React.DOM.input( {type:"radio", value:animalClass, checked:this.sourceKeypath('currentAnimal.animalClass') == animalClass, onChange:this.updateKeypath('currentAnimal.animalClass')} )
                  )
                  )
              })
            ),
            React.DOM.div( {className:"form-group"}, 
              React.DOM.label(null, "Color:"),
              React.DOM.select( {className:"form-control", value:this.sourceKeypath("currentAnimal.color"), onChange:this.updateKeypath("currentAnimal.color")}, 
                this.enumerate("Animal.COLORS", 'color', function(color){
                  return React.DOM.option( {key:color, value:color}, color)
                })
              )
            ),
